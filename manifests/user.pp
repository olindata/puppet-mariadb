...
define mariadb::user ($username, $pw, $dbname, $grants = 'all privileges',
  ...
  if ($dbname == '*') {
    $real_dbname = "*.*"
    $privilege_check_query = "select count(*) from information_schema.USER_PRIVILEGES where GRANTEE=\"'${username}'@'${host_to_grant}'\" and PRIVILEGE_TYPE=\"${grants}\" and IS_GRANTABLE=\"${isgrantable}\""
  } else {
    $real_dbname = "`${dbname}`.*"
    $privilege_check_query = "select count(*) from information_schema.SCHEMA_PRIVILEGES where TABLE_SCHEMA=\"'${dbname}'\" GRANTEE=\"'${username}'@'${host_to_grant}'\" and PRIVILEGE_TYPE=\"${grants}\" and IS_GRANTABLE=\"${isgrantable}"
  }
